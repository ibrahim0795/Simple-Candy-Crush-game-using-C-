...
void ReadImage(string imgname, vector<unsigned char> &imgArray) {
	...
	std::ifstream file(imgname.c_str(), std::ios::binary);
    if (!file) {
        std::cerr << "Error: Unable to open file " << imgname << std::endl;
        return;
    }
    ...
    imgArray.resize(fileSize);

    // Read the file contents into the vector
    file.read(reinterpret_cast<char*>(&imgArray[0]), fileSize);
    file.close();
